...
{
    HAL_NVIC_SetPriority(USART3_IRQn, 15, 0);    // 璁剧疆USART3涓柇鐨勪紭鍏堢骇
    HAL_NVIC_EnableIRQ(USART3_IRQn);            // 浣胯兘USART3鐨勪腑鏂?

		huart3.Instance->SR &= ~(USART_SR_RXNE);
    __HAL_UART_ENABLE_IT(&huart3,UART_IT_RXNE);   // 浣胯兘USRAT3鐨勬帴鏀朵腑鏂?
}
